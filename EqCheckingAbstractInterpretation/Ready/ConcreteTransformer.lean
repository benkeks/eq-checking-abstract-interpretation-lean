...
/-- A concrete least-fixed-point observation is generated by one DRS predecessor step. -/
theorem lfpDRS_postfixpoint
    (env : Env Action Name)
    (p : CCS Action Name)
    (Q : ProcSet Action Name)
    (o : RSObs Action)
    (hLfp : lfpDRS env p Q o) :
    DRS env (lfpDRS env) p Q o := by
  let predecessor : DiffSysRS Action Name (RSObs Action) := DRS env (lfpDRS env)
  have hPrefixed : ∀ p' Q' o', DRS env predecessor p' Q' o' → predecessor p' Q' o' := by
    intro p' Q' o' hStep
    exact dRS_mono env (lfpDRS_prefixpoint env) p' Q' o' hStep
  exact hLfp predecessor hPrefixed
